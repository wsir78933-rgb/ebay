  ...
  "crons": [
    {
      ...
      "schedule": "30 10 * * *"  // UTC 10:30 = 北京时间 18:30 (临时测试)
    }
  ]
}
```
...
|---------|----------|------------|------|
| 10:30 | 18:30 | `30 10 * * *` | ✅ 当前配置 (临时测试) |
| 15:00 | 23:00 | `0 15 * * *` | 原生产配置 |
...
### 2025-10-05 20:37 - 邮件内容增强与返回值修复
- ✨ **新功能**: 邮件显示所有变化商品（移除只显示5个的限制）
- 📧 **增强**: 添加价格变化详情（旧价→新价、涨跌幅度、百分比）
- 📋 **增强**: 添加下架商品完整列表
- 🎨 **改进**: 为每种变化类型添加独特视觉样式和颜色标识
- 🔗 **改进**: 为每个商品添加eBay直链
- 🐛 **修复**: `monitor-sellers.js:477,480` 邮件返回值解析错误
  - 问题: 访问 `result.sendResult.message_id`（不存在）
  - 修复: 改为 `result.messageId`（正确字段）
- ⏰ **调整**: Cron时间临时改为18:30北京时间（UTC 10:30）用于测试
- 📁 **文件**: `api/rube-email-simple.js:202-243`, `api/monitor-sellers.js:477,480`
...
**最后更新**: 2025-10-05 20:37
**维护者**: Claude Code
**项目状态**: ✅ 邮件系统已修复并增强，正常运行
